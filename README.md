...
最後に書くようにMIT Licenseなので、他の方々も自由に使っていただいて構いません。
...
研究室の Overleaf の **New Project → Templates → 学位論文 → 修士論文** を開き、
**Open as Template** を押してください。LuaLaTeX の設定も付いてくるので、
ファイルを持ち込む必要はありません。

**作ったらプロジェクト名を「修士論文（田中太郎）」のように変えてください。**
テンプレートから作るとプロジェクト名が「修士論文」のままになり、
指導教員の画面に全員分が並んだときに区別できません。
...
| `hh-template.sty` | 全テンプレート共通のプリアンブル |
...
* **卒業論文にも同じテンプレートを使えます。** `\thesistype{卒業論文}` や所属を変更してください。
...
講義ノートや手法の解説のように、**一つの話題をまとまった形で書き切る文書**のためのテンプレートです。
...
リポジトリの中で直接書く必要はありません。
`thesis/` `exam/` `note/` はそれぞれ独立しているので、
使いたいものをコピーして持っていってください。

```sh
cp -r ~/Repository/latex-template/exam ~/Documents/class/2026/期末テスト
cd ~/Documents/class/2026/期末テスト
latexmk
```

コピーしたあとは、そのディレクトリだけで完結します。
共通プリアンブル（`hh-template.sty`）も `latexmkrc` も中に入っているので、
元のリポジトリがなくても組めます。

テンプレート側を直したときに、コピーしたほうへ反映する仕組みはありません。
書き始めたら、そのコピーが正本になります。
...
## 共通プリアンブル `hh-template.sty`

フォント、数式、図、単位、執筆中の目印は `hh-template.sty` にまとめてあります。
3 つのテンプレートすべてで同じものを読んでいます。

**原本は `common/hh-template.sty` です。** 各テンプレートにあるのはコピーです。
...
**Terminal.app から実行してください。** 初回だけ聞かれて、あとは
macOS のキーチェーンが覚えます。

エディタや自動化ツールの中で開いたシェルから走らせると、キーチェーンが読めず
`failed to get: -25308` になって毎回聞かれます。
そこからも使いたい場合は `~/.netrc` に資格情報を置く方法がありますが、
**トークンが平文で残ります。** 共有の計算機では避けてください。
