...
% 間違えて latexmk body.tex としたときに、
% 何千行もフォントのエラーを出す前に止めるための仕掛けです。
% ---------------------------------------------------------------------------
\ifdefined\chapter\else
  \errmessage{^^J
   ================================================================^^J
   body.tex is a fragment. It cannot be compiled on its own.^^J
   body.tex は単体ではコンパイルできません。^^J
...
\begin{align}
  \pdif{\rho}{t} &= -\divergence(\rho\bm{v}), \label{eq:continuity}\\
  \pdif{(\rho\bm{v})}{t}
    &= -\divergence(\rho\bm{v}\bm{v})
       - \grad p + \rho\bm{g}
       + \frac{1}{4\pi}(\curl\bm{B})\times\bm{B}, \label{eq:motion}\\
  \pdif{\bm{B}}{t} &= \curl(\bm{v}\times\bm{B}) \label{eq:induction}
\end{align}
